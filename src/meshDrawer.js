...
void main()
{
	...
	float lightImpact = dot(normalizedNormal, normalizedLightDir);
	diff = lightImpact > 0.0 ? lightImpact : 0.0;

	// Calculate Specular Lighting (Phong Reflection Model)
	vec3 viewDirection = normalize(-vPosition);
	vec3 reflectedDirection = reflect(-normalizedLightDir, normalizedNormal);
	float specularHighlight = dot(viewDirection, reflectedDirection);
	spec = specularHighlight > 0.0 ? pow(specularHighlight, phongExp) : 0.0;
